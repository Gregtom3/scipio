    ...
    for(string _input_file: _input_files){
        ...
        while ((key = (TKey*)iter.Next())) {
            if (strstr(key->GetClassName(), "TTree")) {
                ...
                binned_pi0(tree, threshold, Mggmin, Mggmax, smin, smax);
                
                /////////////////////////////Perform azimuthal modulation fits
                sideband_modulations(tree, _input_file, threshold, Mggmin, Mggmax, smin, smax, L );
...
void fitTools::splot_pipluspi0(int L, double threshold, double Mggmin, double Mggmax){
    ...
    for(string _input_file: _input_files){
        ...
        if (strstr(key->GetClassName(), "TTree")) {
            ...
            unbinned_pi0(tree, _input_file, threshold, Mggmin, Mggmax);
            
            /////////////////////////////Perform azimuthal modulation fits
            splot_modulations(tree, threshold, Mggmin, Mggmax, L);
...
void fitTools::unbinned_pi0(TTree *tree , string _input_file, double threshold, double Mggmin, double Mggmax){
...
void fitTools::binned_pi0(TTree *tree,  double threshold, double Mggmin, double Mggmax,
                                  double smin, double smax){
    
    ///////////////////////////// Create directory for saving output
    gSystem->Exec(Form("mkdir -p %s/sdbnd_out/%s/",_input_dir.c_str(),tree->GetName()));
    
    ///////////////////////////// Create histogram 
    TH1F *h = new TH1F("Mdiphoton",";Mdiphoton [GeV];",100,Mggmin,smax);
    
    ///////////////////////////// Draw into the histogram
    tree->Draw("Mgg>>Mdiphoton",Form("prob_g1>%f&&prob_g2>%f&&(hel==1||hel==-1)",threshold,threshold),"goff");
    ...
    f_sdbnd->SetParLimits(2,0.01,0.3);
    ...
    TFile *sdbnd_out = new TFile(Form("%s/sdbnd_out/%s/sideband.root",_input_dir.c_str(),tree->GetName()),"RECREATE");
    TVectorD purity(1);
    purity[0]=u;
...
void fitTools::splot_modulations(TTree *tree, double threshold, double Mggmin, double Mggmax, int L){
...
void fitTools::sideband_modulations(TTree *tree, string _input_file, double threshold, double Mggmin, double Mggmax,
                                    double smin, double smax, int L){
    
    ///////////////////////////// Create directory for saving output
    gSystem->Exec(Form("mkdir -p %s/sdbnd_obs/%s/",_input_dir.c_str(),tree->GetName()));
    
    /////////////////////////////Create azimuthal modulation fit manager
    /////////////////////////////Do this for both the sigbg and bg region (diphoton and sdbnd)
    /*FitManager FM_sigbg;
    FM_sigbg.SetUp().SetOutDir(Form("%s/sdbnd_obs/%s/sigbg",_input_dir.c_str(),tree->GetName()));
    process_azi_FM(FM_sigbg,threshold,Mggmin,Mggmax,L);
    FM_sigbg.LoadData(tree->GetName(),_input_file.c_str());
    Here::Go(&FM_sigbg);*/
    
    FitManager FM_bg;
    FM_bg.SetUp().SetOutDir(Form("%s/sdbnd_obs/%s/bg",_input_dir.c_str(),tree->GetName()));
    process_azi_FM(FM_bg,threshold, smin, smax, L);
    FM_bg.LoadData(tree->GetName(),_input_file.c_str());
    Here::Go(&FM_bg);
